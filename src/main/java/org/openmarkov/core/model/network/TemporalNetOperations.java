                ...
                if (!probNet.containsShiftedVariable(generatingNode.getVariable(), 1)) {
                    ...
                    if (!node.getPotentials().isEmpty() && node.getPotentials().get(0) instanceof SameAsPrevious) {
                        ...
                        newPotential.shift(probNet,
                                           variable.getTimeSlice() - newPotential.getConditionedVariable()
                                                                                 .getTimeSlice());
...
//							TablePotential previousCyclePotential = (TablePotential) previousCyclePotentials.get(i);
        ...
        for (Node utilityNode : utilityExpandedNodes) {
            ...
            if (utilityVariable.isTemporal()) {
                ...
                double discount = CycleLength.getTemporalAdjustedDiscount(probNet.getCycleLength().getUnit(),
                                                                          probNet.getCycleLength()
                                                                                 .getValue(), utilityVariable.getDecisionCriterion()
                                                                                                             .getDiscountUnit(),
                                                                          utilityVariable.getDecisionCriterion()
                                                                                         .getDiscount());
     ...
     */
    public static ProbNet expandNetwork(ProbNet probNet, EvidenceCase preResolutionEvidence, String networkName) throws IncompatibleEvidenceException.EvidenceIsIncompatibleWithOther, NonProjectablePotentialException {
        //FIXME hardcoded
...
//	}
    ...
    public static void transformToInfluenceDiagram(ProbNet expandedNetwork) {
        ...
        temporalNodes.forEach(node -> {
            node.getVariable()
                .setName(node.getVariable().getBaseName() + " |" + node.getVariable().getTimeSlice() + "|");
