...
public class SystematicSampling extends Sampler {
				...
				for (TreeADDBranch branch : ((TreeADDPotential) pot).getBranches()) {
					if (branch != null) {
						...
						if (branchPotential != null) {
							Hashtable<UncertainValue, SubPotentialAndPosition> auxUncertainValues = getUncertainValues(branchPotential);
	...
	private static void replace(TreeADDPotential potential,
			TablePotential subPotToReplace,
			TablePotential newSubPot) {	
	...
	private static void replace(TreeADDBranch treeADDBranch,
			TablePotential subPotToReplace, TablePotential newSubPot) {
