...
public class TablePotential extends Potential implements Comparable {
	...
	/**
	 * @param variables
	 *            . <code>ArrayList</code> of <code>Variable</code> used to
	...
	public TablePotential(Potential potential) throws NotEnoughMemoryException {
	...
	private TablePotential(ArrayList<Variable> variables, PotentialRole role,
			double[] table, int initialPosition, int[] offsets, int[] dimensions) {
		super(variables, role);
		this.originalVariables = this.variables;
		this.values = table;
	...
	public ArrayList<TablePotential> tableProject(EvidenceCase evidenceCase,
		...
		ArrayList<Variable> unobservedVariables = (ArrayList<Variable>) variables
				.clone();
		...
		if (numVariables == numUnobservedVariables) { // No projection.
			...
			if (hasUncertainTable) {
				projectedPotential
						.setUncertainTable(new UncertainValue[length]);
			}
			...
			if (numUnobservedVariables == 0) {// Projection = constant potential
				...
				int[] accumulatedOffsets = projectedPotential
						.getAccumulatedOffsets(variables);
				int numVariablesProjected = projectedPotential
						.getNumVariables();
				...
				for (int i = 0; i < numVariablesProjected; i++) {
					projectedDimensions[i] = unobservedVariables.get(i)
							.getNumStates();
				}
		...
		if (role == PotentialRole.UTILITY && inferenceOptions != null
			...
			double discount = Math
					.pow(inferenceOptions.discountRate, timeSlice);
		...
		if (role == PotentialRole.UTILITY && inferenceOptions != null) {
			...
			String criterion = probNet.getProbNode(utilityVariable)
					.getPurpose();
			if (role == PotentialRole.UTILITY && decisionCriteria != null) {
				...
				try {
					potentials.add(decisionCriteria
							.deltaTablePotential(criterion));
				} catch (InvalidStateException e) {
					throw new WrongCriterionException(utilityVariable,
							criterion, decisionCriteria);
				}
				// System.out.println("vamos a multiplicar");
				projectedPotential = DiscretePotentialOperations
						.multiply(potentials);
			}
		}
	...
	public int[] getAccumulatedOffsets(ArrayList<Variable> otherVariables) {
		...
		for (int i = 1; i < offsetX.length; i++) {
			offsetX[i] = offsetX[i - 1]
					* otherVariables.get(i - 1).getNumStates();
		}
		...
		for (int j = 1; j < accOffsetXY.length; j++) {
			...
			int numStatesYj_1 = ((Variable) variables.get(j - 1))
					.getNumStates();
	...
	private int getPosition(EvidenceCase configuration) {
		...
		for (int i = startLoop; i < sizeCoordinates; i++) {
			coordinates[i] = configuration.getFinding(varsTable.get(i))
					.getStateIndex();
		}
	...
	public Collection<Finding> getInducedFindings(EvidenceCase evidenceCase)
		...
		if (role == PotentialRole.CONDITIONAL_PROBABILITY
			...
			TablePotential projectedPotential = tableProject(evidenceCase, null)
					.get(0);
			if ((projectedPotential.getNumVariables() == 1)
				...
				if (zeros == (table.length - 1)) {// new finding
					inducedFindings.add(new Finding(projectedPotential
							.getVariable(0), position));
				}
	 ...
	 */
	public static boolean checkUncertainTable(
			ArrayList<UncertainValue> uncertainTable) {
	...
	public Potential shift(ProbNet probNet, int timeDifference)
		...
		ArrayList<Variable> shiftedVariables = getShiftedVariables(probNet,
				timeDifference);
		TablePotential shiftedPotential = new TablePotential(shiftedVariables,
				role);
		if (role == PotentialRole.UTILITY) {
			shiftedPotential.setUtilityVariable(probNet.getShiftedVariable(
					utilityVariable, timeDifference));
		}
	...
	public Potential sample(Variable simulationIndexVariable)
		...
		if (simulationIndexVariable != null) {
			SamplePotentialTable samplePotentialTable = new SamplePotentialTable(
					this, simulationIndexVariable);
	...
	public Potential copy() throws NotEnoughMemoryException {
		TablePotential newPotential = new TablePotential(
				new ArrayList<Variable>(variables), role);
