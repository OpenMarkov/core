...
public class StrategyTree extends TreeADDPotential implements Cloneable {
    ...
    protected static boolean equalInterventions(StrategyTree[] strategyTrees) {
        if (strategyTrees.length == 0) {
            return true;
        }
        StrategyTree firstStrategyTree = strategyTrees[0];
        for (int i = 1; i < strategyTrees.length; i++) {
            StrategyTree argumentTree = strategyTrees[i];
            boolean bothAreEqual = firstStrategyTree == null ? argumentTree == null : firstStrategyTree.equals(argumentTree);
            if (!bothAreEqual) {
                return false;
            }
        }
    ...
    public StrategyTree concatenate(StrategyTree strategyTree) {
        ...
        for (TreeADDBranch branch : branches) {
            ...
            if (oldStrategyTree == null) {
                ...
                branchStrategyTree.concatenate(strategyTree);
            }
        }
        return this;
    }
        ...
        for (StrategyTree skNode : nodes) {
            ...
            if (skNode.topVariable != null) {
                ...
                if (leaves.contains(skNode)) {
                    ...
                    if ((skNodeBranches != null) && (!skNodeBranches.isEmpty())) {
    ...
    private static String getStringStates(List<State> states) {
        String str = "";
    ...
    private static String toStringShapeForGraphviz(ProbNet net, Variable topVariable) {
        String string = null;
        
        if (net != null) {
            Node node = net.getNode(topVariable.getName());
            if (node != null) {
    ...
    private static String toStringColorShapeForGraphviz(ProbNet net, Variable topVariable) {
        String string = null;
