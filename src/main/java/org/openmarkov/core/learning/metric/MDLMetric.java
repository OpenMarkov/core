...
public class MDLMetric extends EntropyMetric {
     ...
     */
    public MDLMetric(ProbNet probNet, int[][] cases) 
    ...
    public double score() throws NotEnoughMemoryException {
        return calculateEntropy() - (calculateDimension()/2) * Math.log
                (cases.length);
    }
    ...
    protected double score(AddLinkEdit edition, boolean change) 
        ...
        int totalNumberOfCases = cases.length;
        ...
        return ((newNodeEntropy - (newNodeDimension / 2) * 
                Math.log(totalNumberOfCases)) - (lastNodeEntropy - 
                (lastNodeDimension / 2) * 
                Math.log(totalNumberOfCases)));
    }
    ...
    protected double score(InvertLinkEdit edition, boolean change) 
        ...
        lastNodeEntropy = nodesEntropies.get(initialDestinationNode.getName());
        newNodeEntropy = nodeEntropyRemovedLink(initialDestinationNode, 
        		initialOriginNode, false);
        
        lastNodeDimension = 
        	nodesDimensions.get(initialDestinationNode.getName());
        newNodeDimension = lastNodeDimension / ((edition.
                getVariable1()).getNumStates());
        ...
        result = ((newNodeEntropy - (newNodeDimension / 2) * 
                Math.log(totalNumberOfCases)) - (lastNodeEntropy - 
                (lastNodeDimension / 2) * Math.log(totalNumberOfCases)));
