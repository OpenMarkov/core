...
import org.openmarkov.core.model.network.constraint.compound.CompoundConstraint;
import org.openmarkov.core.model.network.constraint.compound.NetworkTypeConstraint;
import org.openmarkov.core.model.network.constraint.compound.NoNetworkTypeConstraint;
...
public class ProbNet implements Cloneable {
	...
	public ProbNet(NetworkTypeConstraint networkTypeConstraint) {
		...
		nodesHashMaps = new ArrayList<LinkedHashMap<Variable, ProbNode>>(
				numNodeTypes);
		for (int i = 0; i < numNodeTypes; i++) {
			nodesHashMaps.add(new LinkedHashMap<Variable, ProbNode>());
		}

		pNESupport = new PNESupport(this, false);
		
		try {
			if (networkTypeConstraint != null){
				this.addConstraint(networkTypeConstraint, false);
			} else{
				this.addConstraint(
						NoNetworkTypeConstraint.getUniqueInstance(), false);
			}
		} catch (ConstraintViolationException e) {
			System.err.println("ConstraintViolationException in ProbNet" + 
			" constructor");
		}
	...
	public void addConstraint(PNConstraint constraint, boolean check)
		...
		if (check && !constraint.checkProbNet(this)) {
			throw new ConstraintViolationException("Can not apply "
					+ constraint.toString() + " to this probNet.");
		} else {
	...
	/**
	 * Sets Network type constraint. There is only one and it is stored in first
	 * position.
	 * 
	 * @param constraint
	 *            . <code>NetworkTypeConstraint</code>
	 */
	public void setNetworkTypeConstraint(
			NetworkTypeConstraint networkTypeConstraint) {

		if (constraints.size() > 0) {
			constraints.set(0, networkTypeConstraint);
			pNESupport.addUndoableEditListener(networkTypeConstraint);
		} else {
			constraints.add(networkTypeConstraint);
			pNESupport.addUndoableEditListener(networkTypeConstraint);
		}
	 ...
	 * position.
	 * 
	 * @return constraint. <code>NetworkTypeConstraint</code>
	 */
	public NetworkTypeConstraint getNetworkTypeConstraint() {
		NetworkTypeConstraint networkConstraint = null;
		if ((constraints != null) && (constraints.size() > 0)) {
			networkConstraint = (NetworkTypeConstraint)constraints.get(0);
		}
	...
	public ProbNet copy() {
		// copy constraints
		NetworkTypeConstraint networkType = null;
		int numConstraints = constraints.size(); 
		if (numConstraints > 0) {
			networkType = (NetworkTypeConstraint)constraints.get(0);
		}
		ProbNet probNetCopy = new ProbNet(networkType);
		for (int i = 1; i < numConstraints; i++) {
			try {
				probNetCopy.addConstraint(constraints.get(i), 
						false); // Do not test
			} catch (ConstraintViolationException e) {
	 ...
	 * @throws NoFindingException
	 */
	public ArrayList<TablePotential> tableProjectPotentials(
	...
	throws NotEnoughMemoryException, NonProjectablePotentialException, WrongCriterionException {
		...
		for (Potential potential : originalPotentials) {
			...
			for (Potential auxPotential : auxPotentials) {
				projectedPotentials.add((TablePotential)auxPotential);
			}
		}
		return projectedPotentials;
	}
	...
	public boolean hasConstraint(Class constraint) {
		for (PNConstraint constraintProbNet : constraints) {
				...
				if (constraintProbNet.getClass() == constraint) {
					return true;
				} else if (constraintProbNet instanceof CompoundConstraint) {
					if (((CompoundConstraint) constraintProbNet)
							.hasConstraint(constraintProbNet.getClass())) {
						return true;
					}
				}
