...
/** This class stores an <code>ArrayList</code> of <code>Findings</code> and
 * can search them with the name.
 * @author marias
 * @author fjdiez
...
public class EvidenceCase {
    ...
    // Constructors
    /** @param findings <code>HashMap</code> of key=<code>String</code> and 
     *  value=<code>Finding</code>. */
    ...
    throws InvalidStateException, IncompatibleEvidenceException {
    	if (isCompatible(finding)) {
    		...
    		throw new IncompatibleEvidenceException("Error trying to add " +
    				"evidence: " + finding.toString() + " having previously " +
    ...
    public void addFinding(
    		ProbNet probNet, String variableName, String stateName) 
	throws ProbNodeNotFoundException, InvalidStateException, 
     ...
     * @throws ProbNodeNotFoundException */
    public void addFinding(
    		ProbNet probNet, String variableName, double value) 
	throws ProbNodeNotFoundException, InvalidStateException, 
	IncompatibleEvidenceException {
    	Variable variable = probNet.getVariable(variableName);
	...
	/** @return <code>true</code> if there are no findings. 
	 *  <code>boolean</code> */
	 ...
	 *  It writes the name of the variables and the findings. */
	...
	public void extendEvidence(ProbNet probNet, double cycleLength) 
		...
		while (!pendingFindings.isEmpty()) {
			...
			for (Potential potential : potentials) {
				...
				pendingFindings.addAll(newFindings);
			}
		}
	}
	...
	throws InvalidStateException {
		...
		if (existingFinding == null) {
			...
			case DISCRETIZED:
				...
				if (variable.getStateIndex(existingFinding.getNumericalValue())
						== existingFinding.stateIndex) {
					if (newFinding.numericalValue != 
						existingFinding.numericalValue) {
						return false;
					}
				}
			}
	...
	public EvidenceCase shiftEvidenceBackwards(
			int timeDifference, ProbNet probNet) {
		...
		try {
			for (Finding finding : findings.values()) {
				...
				if (findingVariable.isTemporal()) {
					// do not generate variables with negative timeSlice
					if (findingVariable.getTimeSlice() - timeDifference >= 0) {
						Variable shiftedVariable = probNet.getShiftedVariable(
								findingVariable, -timeDifference);
						if (probNet.containsVariable(shiftedVariable.getName())) {
							Finding shiftedFinding;
							shiftedFinding = new Finding(
								shiftedVariable, finding.stateIndex);
							shiftedFinding.numericalValue = 
								finding.numericalValue;
