...
public abstract class OpenMarkovException2 extends OpenMarkovException {
     ...
     * {@link OpenMarkovException2}.
     */
    protected final @Nullable String autoGetExceptionTitle() {
        return OpenMarkovException2.localizeWithOpenMarkov(this, BundleSearch.BUNDLE_SUFFIX_TITLE);
    }
    ...
    protected final @Nullable String autoGetExceptionMessage() {
        return OpenMarkovException2.localizeWithOpenMarkov(this, BundleSearch.BUNDLE_SUFFIX_MESSAGE);
    }
    
    /**
     * Turns any kind of exception to an {@link OpenMarkovException2}, the conversion varies depending on the Exception
     * sent:
     * <ul>
     *   <li>If it is already a {@link OpenMarkovException2}: it returns said exception</li>
     ...
     * </ul>
     *
     * @param exception The exception to turn into {@link OpenMarkovException2}.
     * @return an exception turned into {@link OpenMarkovException2}.
     */
    @SuppressWarnings("unchecked")
    public static @NotNull OpenMarkovException2 of(@Nullable Exception exception) {
        if (exception instanceof OpenMarkovException2 alreadyPrepared) {
    ...
    /**
     * It attempts to find a localized string by the name of the {@link OpenMarkovException2}'s class.
     * <p>
     * If found, it will also format said String using the fields of the class as parameters.
     *
     * @param openMarkovException2 the exception to localize
    ...
    private static @Nullable String localizeWithOpenMarkov(OpenMarkovException2 openMarkovException2, String suffixKey) {
        ...
        var classForFields = openMarkovException2.getClass();
        while (classForFields != OpenMarkovException2.class) {
            Arrays.stream(classForFields.getDeclaredFields()).forEach(field -> {
                try {
                    field.setAccessible(true);
                    if (!fieldsAndValues.containsKey(field.getName())) {
                        fieldsAndValues.put(field.getName(), field.get(openMarkovException2));
                    }
                } catch (IllegalAccessException ex) {
                    LOGGER.warn(ex);
                }
            });
            classForFields = (Class<? extends OpenMarkovException2>) classForFields.getSuperclass();
        }
    ...
    static private class BundleSearch {
         ...
         */
        static Optional<ExceptionBundleInfo> findExceptionBundle(Class<? extends OpenMarkovException2> exceptionClass) {
            String exceptionClassName = exceptionClass.getName();
