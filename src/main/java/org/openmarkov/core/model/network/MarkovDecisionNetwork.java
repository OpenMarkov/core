		...
		for (int i = 0; i < numVariablesToRemove; i++) {
			...
			if (nodeType == NodeType.DECISION) {
				...
				if (parentsOfDecisions.get(index).size() > 0) {
					partialOrder.add(parentsOfDecisions.get(index));
				}
				ArrayList<Variable> oneDecisionArray = 
					new ArrayList<Variable>();
				oneDecisionArray.add(variable);
				partialOrder.add(oneDecisionArray);
			}
...
/*	*//** @return An <code>ArrayList</code> of <code>ArrayList</code> of 
    ...
    private void addLinks(Potential potential) {
		...
		for (int i = 0; i < potentialSize - 1; i++) {
			Node node1 = 
				getProbNode(variablesPotential.get(i)).getNode();
			for (int j = i + 1; j < potentialSize; j++) {
    			Node node2 = 
    				getProbNode(variablesPotential.get(j)).getNode();
