...
public class PartialOrder {
     ...
     * WrongGraphStructureException { order = new
     ...
     * node to this MarkovDecisionNetwork
     * 
     * //mluque: I will probably have to uncomment the next line
     * //addVariable(variable, nodeType); // adds decision nodes and its parents
     * to partialOrder if (nodeType == NodeType.DECISION) { int index =
     * decisionVariables.indexOf(variable); if
     * (parentsOfDecisions.get(index).size() > 0) {
     * order.add(parentsOfDecisions.get(index)); } ArrayList<Variable>
     * oneDecisionArray = new ArrayList<Variable>();
     * oneDecisionArray.add(variable); order.add(oneDecisionArray); }
     * idCopy.removeProbNode(node); // remove from influenceDiagram } if
     * (unobservableVariables.size() > 0) { order.add(unobservableVariables); }
     * 
     * 
     * 
    ...
    /**
     * If there is a chance node without parents or without children, returns
     * that node. Otherwise, if there is just one decision node without nodes,
     * returns that node. Otherwise, i.e., if there are more than one decision
     * nodes without parents, throws an exception.
     * 
    ...
    private ProbNode getNextNodeToDelete(ProbNet influenceDiagram)
        ...
        List<ProbNode> chanceNodes = influenceDiagram.getProbNodes(NodeType.CHANCE);
        ...
        List<ProbNode> decisionNodes = influenceDiagram.getProbNodes(NodeType.DECISION);
     ...
     *            <code>List</code> of <code>ArrayList</code> of
     *            <code>Variable</code>
     * @return An <code>List</code> of <code>Variable</code> that are not
     *         parents of any decision.
     */
    private List<Variable> getUnobservableVariables(List<Variable> chanceVariables,
            List<ArrayList<Variable>> parentsVariables) {
        List<Variable> unobservableVariables = new ArrayList<Variable>(chanceVariables);
        for (List<Variable> variables : parentsVariables) {
    ...
    private List<List<Variable>> getParentsOfDecisions(ProbNet influenceDiagram,
            List<Variable> decisionVariables) {
        List<List<Variable>> parentsVariables = new ArrayList<>();
        for (Variable decision : decisionVariables) {
            ...
            List<Node> nodeParents = probNode.getNode().getParents();
            List<Node> nodeParentCloned = new ArrayList<Node>(nodeParents);
            for (Node node : nodeParentCloned) {
                if (((ProbNode) node.getObject()).getNodeType() == NodeType.DECISION) {
                    nodeParents.remove(node);
                }
            }
            List<Variable> parentVariables = influenceDiagram.getVariables(nodeParents);
