...
public class AddProbNodeEdit extends SimplePNEdit {
	 ...
	 *            
	 */
	public AddProbNodeEdit(ProbNet probNet, String newNodeName, NodeType nodeType, 
	...
	public void doEdit() {
		...
		if (nodeType == NodeType.UTILITY){
			newVariable = new Variable (newNodeName);
		} else {
			newVariable = new Variable (newNodeName,states);
		}
		
		//Adds the new variable to network ( creates a probNode instance ) 
		newNode = probNet.addVariable(newVariable, nodeType);
		...
		if (nodeType != NodeType.DECISION){
			Potential  uniformPotential = PotentialOperations.
			getUniformPotential(probNet, newVariable, nodeType);		
			probNet.addPotential(uniformPotential);
		}else{
