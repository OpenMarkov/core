...
{  
    ...
    public final Class<?> findLearningAlgorithmByName (String name)
    {
        try
        {
            ...
            for (Class<?> plugin : plugins) {
                LearningAlgorithm lAnnotation = plugin.getAnnotation (LearningAlgorithm.class);
    ...
    public final  List<Class<?>> findAllLearningAlgorimths ()
    {
        try
        {
            FilterIF filter = org.openmarkov.plugin.Filter.filter().toBeAnnotatedBy (LearningAlgorithm.class);
